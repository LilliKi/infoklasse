...
let skalaY = 670;
let data;
let years = 0; // muss variabel sein --> abhängig von angeklickter jahreszahl;
let year;
let smallI;
let largeI;
let buttonYear;
...
function setup() {
  ...
  // Buttons
   ...
   let abstand = map(1, 0, data.getRowCount() - 1, 0, 900);
   
   for (let i = 0; i < data.getRowCount(); i++) {
    buttonYear = data.getNum(i,1);
    let button = createButton(buttonYear);
    button.position(50 + i * abstand, skalaY);
    button.size(abstand-2,25);
    button.mousePressed(numberChange1998);
